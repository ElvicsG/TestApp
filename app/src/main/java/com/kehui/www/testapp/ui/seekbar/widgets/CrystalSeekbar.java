package com.kehui.www.testapp.ui.seekbar.widgets;
...
import com.kehui.www.testapp.ui.seekbar.interfaces.OnSeekbarChangeListener;
import com.kehui.www.testapp.ui.seekbar.interfaces.OnSeekbarFinalValueListener;
...
public class CrystalSeekbar extends View {
    ...
    public void setOnSeekbarFinalValueListener(OnSeekbarFinalValueListener
                                                       onSeekbarFinalValueListener) {
    ...
    public float getThumbWidth() {
        return (leftThumb != null) ? leftThumb.getWidth() : getResources().getDimension(R.dimen
                .thumb_width);
    }

    public float getThumbHeight() {
        return (leftThumb != null) ? leftThumb.getHeight() : getResources().getDimension(R.dimen
                .thumb_height);
    }
    ...
    public void apply() {
        ...
        thumbWidth = (leftThumb != null) ? leftThumb.getWidth() : getResources().getDimension(R
                .dimen.thumb_width);
        thumbHeight = (leftThumb != null) ? leftThumb.getHeight() : getResources().getDimension(R
                .dimen.thumb_height);
    ...
    protected int getBarHighlightColor(final TypedArray typedArray) {
        return typedArray.getColor(R.styleable.CrystalRangeSeekbar_bar_highlight_color, Color
                .BLACK);
    }
    ...
    protected void setupLeftThumb(final Canvas canvas, final Paint paint, final RectF rect) {
        ...
        leftThumbColor = (Thumb.MIN.equals(pressedThumb)) ? leftThumbColorPressed :
                leftThumbColorNormal;
        paint.setColor(leftThumbColor);

        rectLeftThumb.left = normalizedToScreen(normalizedMinValue);
        rectLeftThumb.right = Math.min(rectLeftThumb.left + (getThumbWidth() / 2) + barPadding,
                getWidth());
    ...
    protected void drawLeftThumbWithColor(final Canvas canvas, final Paint paint, final RectF
            rect) {
        canvas.drawOval(rect, paint);
    }

    protected void drawLeftThumbWithImage(final Canvas canvas, final Paint paint, final RectF
            rect, final Bitmap image) {
    ...
    private <T extends Number> Number formatValue(T value) throws IllegalArgumentException {
        ...
        throw new IllegalArgumentException("Number class '" + value.getClass().getName() + "' is " +
                "not supported");
    }
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(getMeasureSpecWith(widthMeasureSpec), getMeasureSpecHeight
                (heightMeasureSpec));
    }
